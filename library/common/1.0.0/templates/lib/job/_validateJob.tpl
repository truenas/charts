{{- define "ix.v1.common.validate.job" -}}
  ...
  {{- if not $job.podSpec -}}
    {{ fail (printf "A pod in <podSpec> is required in <job> (%s)." $name) }}
  {{- end -}}

  {{- if not $job.podSpec.containers -}}
    {{ fail (printf "At least one container in <podSpec.containers> is required in <job> (%s)." $name) }}
  {{- end -}}
{{- end -}}
